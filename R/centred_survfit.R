centred_surv <- function (formula, newdata, se.fit = TRUE, predvar = NULL, conf.int = 0.95, individual = FALSE,
          stype = 2, ctype, conf.type = c("log", "log-log", "plain",
                                          "none", "logit", "arcsin"), censor = TRUE, start.time,
          id, influence = FALSE, na.action = na.pass, type, ...)
{
  ...
  if (!missing(type)) {
    if (!missing(stype) || !missing(ctype))
      ...
      if (stype != 1)
        ctype <- c(1, 1, 2, 1, 1, 2, 1, 1, 1)[survtype]
    }
  }
  if (missing(ctype)) {
  ...
  if (is.null(mf)) {
    ...
    if (is.null(Y) || coxms) {
      Y <- model.response(mf)
      if (is.null(object$timefix) || object$timefix)
        Y <- aeqSurv(Y)
    }
  ...
  if (individual) {
    ...
    # ----------------------------------------------------- new data model matrix creation

    x2 <- model.matrix(Terms2, mf2)[, -1, drop = FALSE]
    if (!is.null(predvar)){


      # center at population mean

      m_base <- model.frame(Terms2, model.frame(object)) %>% as_tibble() %>% mutate(nxxxx = 1)

      centered <- summary_table(m_base, "n") %>%
        filter(var != "nxxxx", !var %in% predvar) %>%
        mutate(level = ifelse(is.na(level), "", level)) %>%
        transmute(lvl = paste(var,level, sep = ""), mean)


      x2[,colnames(x2) %in% centered$lvl] <- rep(centered$mean[match(colnames(x2[,colnames(x2) %in% centered$lvl]), centered$lvl)], each = nrow(x2))
      # print(x2)
    }
  ...
  if (se.fit && conf.type != "none") {
    ci <- survival:::survfit_confint(result$surv, result$std.err, logse = result$logse,
                          conf.type, conf.int)
